...
export default function BloodStockPage() {
  ...
  useEffect(() => {
    const loadStock = async () => {
      setLoading(true)
      const { data } = await supabase.from('blood_stock').select('*').order('blood_type')
      setBloodStock(data || [])
      setLoading(false)
    }
    loadStock()
  ...
  const stockTrends = bloodStock.map((stock) => {
  ...
  const averageUnits = bloodStock.length > 0 ? Math.round(totalUnits / bloodStock.length) : 0
    ...
    <div className="min-h-screen bg-secondary">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {loading ? (
            <p className="text-text-muted">Loading blood stock...</p>
          ) : (
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-foreground">{stock.blood_type}</p>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-4 border-t border-border border-opacity-50">
                    <div>
                      ...
                      <p className="text-sm font-medium text-foreground">{stock.last_updated}</p>
                    </div>
                    <div>
                      <p className="text-xs text-text-muted mb-1">Expiry Date</p>
                      <p className="text-sm font-medium text-foreground">{stock.expiry_date}</p>
                    </div>
                  </div>
